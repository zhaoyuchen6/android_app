...
export default {
  ...
  computed: {
    ...
    style () {
      return {
        height: this.height/12.8 + 'vw'
      }
    },

    rowCount () {
      return Math.floor(this.boxWidth / this.cellWidth)
    },

    rowShift () {
      if (this.center) {
        let contentWidth = this.items.length * this.cellWidth
        let rowShift = contentWidth < this.boxWidth
          ? (this.boxWidth - contentWidth) / 2
          : (this.boxWidth % this.cellWidth) / 2
